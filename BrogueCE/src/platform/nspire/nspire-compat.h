 ...
 * the POSIX environment that BrogueCE normally targets.
 *
...
/*
 * mkdir: Ndless newlib provides mkdir() in <sys/stat.h> but its signature
 * is mkdir(const char *path, mode_t mode).  We just need to ensure the
 * header is visible — no wrapping required.
 */
