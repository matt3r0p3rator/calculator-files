 ...
 *   /documents/brogue/BrogueHighScores.txt.tns
 *   /documents/brogue/mysave.broguesave.tns
 */
...
FILE *nspire_fopen(const char *path, const char *mode)
{
    ...
    if (mode[0] == 'r') {
        /* Try exact path first (handles already-.tns paths), then with .tns. */
