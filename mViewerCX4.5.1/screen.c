...
void switchScrOffOn(uint8_t s)
{
	...
	if(s) {
		...
		mode &= ~mask;
		*(volatile unsigned*) SCREEN_CONTRAST_ADDR=(contrast&~0b11111111);
	}
...
void dispBufIMG(unsigned short int* buf, int xoff, int yoff, unsigned short int* img, int width, int height, float inc)
{	int dwidth=width, dheight=height;
	int data_x=0, data_y=0;
	unsigned int x,y;
	dwidth = (int)(dwidth/inc);
	dheight = (int)(dheight/inc);
	if(xoff < 0){
		dwidth = dwidth + xoff;
		data_x = (int)(-xoff*inc);
		xoff = 0;
	...
	dwidth = min(dwidth,SCREEN_WIDTH);
	dheight = min(dheight,SCREEN_HEIGHT);
	for(x=xoff; x < dwidth; x++)
		for(y=yoff; y < dheight; y++) {
			setBufPixel(buf, x, y, img[((((int)(y*inc))+data_y)*width+((int)(x*inc))+data_x)]);
		}
}
